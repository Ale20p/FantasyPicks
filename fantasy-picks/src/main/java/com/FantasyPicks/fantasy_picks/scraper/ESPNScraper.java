 ...
 *   2. Use the X-Fantasy-Filter header to request players sorted by STANDARD draft rank
...
public class ESPNScraper implements RankingScraper {
    ...
    private static final String API_TEMPLATE =
            "https://lm-api-reads.fantasy.espn.com/apis/v3/games/ffl/seasons/%d/segments/0/leaguefree?view=kona_player_info";

    /** Filter header requesting players sorted by STANDARD draft rank, top 300 */
    private static final String FANTASY_FILTER_TEMPLATE =
    ...
    private List<PlayerRanking> parseResponse(String body) throws ScrapingException {
        try {
            ...
            for (JsonNode entry : playersNode) {
                ...
                if (playerNode.isMissingNode()) continue;
